...
import { logout } from "./account/actions/auth";
import { getUser } from "./lib/auth/dal";
import SignInForm from "./ui/auth/signin-form";
import SignInOrSignUpForm from "./ui/auth/signin-or-signup-form";
			...
			<div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
				<main className="flex flex-col gap-[32px] row-start-2 items-center justify-center sm:items-start">
					<div className="flex flex-col items-center justify-center gap-1">
						<h1
							className={`p-4 text-3xl md:text-6xl text-center bg-clip-text bg-gradient-to-r from-primary to-tertiary text-transparent ${fasterOne.className}`}
						>
							Django Overtuned
						</h1>
						<p> x </p>
						...
						{user ? (
							<>
								<h1 className="text-2xl font-bold">
									Welcome back {user.data.user.display} !
								</h1>
								<button
									type="button"
									className="rounded-full border border-solid border-transparent transition-colors flex items-center justify-center bg-foreground text-background gap-2 hover:bg-[#383838] dark:hover:bg-[#ccc] font-medium text-sm sm:text-base h-10 sm:h-12 px-4 sm:px-5 sm:w-auto"
